...
## 10. Neo4j 配置方式
默认关闭 Neo4j，系统使用 MySQL 图谱降级实现；启用后，创建图谱版本时会自动将当前图谱快照同步到 Neo4j。
...
- `sync-on-startup=true` 时，后端启动后会自动将当前 MySQL 图谱快照同步到 Neo4j
- 当 Neo4j 不可用或未同步图谱快照时，自动降级到 MySQL 图谱查询
- 图谱版本创建后会自动同步当前三元组到 Neo4j，图谱检索页会优先走 Neo4j 真实查询
- Cypher 查询入口支持直接执行 Neo4j 语句，并返回结构化结果
- 如不想依赖后端同步，可直接在 Neo4j Browser 中执行 `sql/03_neo4j_snapshot.cypher`
...
5. Neo4j 作为图查询增强层，不替代 MySQL 业务主库

## 12. 重点演示路径建议
1. 管理员登录 -> 首页看板
2. 预案管理 -> 新增/查看预案 -> 执行文本分段
3. 抽取任务 -> 新建任务 -> 执行任务
4. 图谱中心 -> 按区域/预案查看实体与三元组 -> 图谱检索可视化
5. 问答推理 -> 选择区域或预案，输入问题展示问答与推理链
6. 质量评估 -> 处理冲突 / 应用补全
7. 版本管理 -> 创建图谱版本 -> 同步 Neo4j 图数据库
