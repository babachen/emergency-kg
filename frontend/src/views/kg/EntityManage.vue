...
import { getRegions } from '../../api/document'
import { getEntities, getEntityTypes } from '../../api/kg'
const query = reactive({ current: 1, pageSize: 20, regionId: undefined, entityTypeId: undefined, keyword: '' })
const tableData = reactive({ total: 0, records: [] })
const entityTypes = ref([])
const regions = ref([])
const loadData = async () => Object.assign(tableData, await getEntities(query))
onMounted(async () => { regions.value = await getRegions(); entityTypes.value = await getEntityTypes(); loadData() })
</script>
